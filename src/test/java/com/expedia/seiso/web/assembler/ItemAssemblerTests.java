...
import com.expedia.seiso.domain.entity.InfrastructureProviderRegion;
...
public class ItemAssemblerTests {
	...
	@Mock private EntityLinks links;
	@Mock private ItemMetaLookup itemMetaLookup;
	
	// Test data
	private InfrastructureProviderRegion region;
	@Mock private ProjectionNode regionQuery;
	@Mock private PersistentEntity regionMeta;
	@Mock private PersistentProperty regionKeyProp;
	@Mock private LinkBuilder linkBuilder;
	@Mock private Link link;
	@Mock private ItemMeta itemTypeMeta;
	...
	private void initTestData() {
		this.region = new InfrastructureProviderRegion().setKey("amazon-us-west").setName("Amazon US West");
		
		when(regionMeta.getPersistentProperty("key")).thenReturn(regionKeyProp);
		when(linkBuilder.withSelfRel()).thenReturn(link);
		when(itemMetaLookup.getItemMeta((Class<InfrastructureProviderRegion>) anyObject())).thenReturn(itemTypeMeta);
	}
	
	private void initDependencies() {
		when((PersistentEntity) repositories.getPersistentEntity(InfrastructureProviderRegion.class)).thenReturn(regionMeta);
		when(links.linkForSingleResource(eq(InfrastructureProviderRegion.class), anyString())).thenReturn(linkBuilder);
	}
