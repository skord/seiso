...
import com.expedia.seiso.web.dto.AuditData;
...
public class ItemAssembler {
	...
	public MapItemDto toDto(Item item, @NonNull final ProjectionNode projectionNode) {
		...
		// FIXME For now suppress audit data, since we're not really using it, and it's appearing where we don't want
		// it. Need to control this with the projections I think. [WLW]
//		val auditData = toAuditData(item);
		val auditData = (AuditData) null;
		
		return new MapItemDto(selfUri, item.getId(), model, auditData);
	}
	...
	private AuditData toAuditData(Item item) {
		val createdBy = item.getCreatedBy();
		val createdByUsername = (createdBy == null ? null : createdBy.getUsername());
		val createdDate = item.getCreatedDate();
		val updatedBy = item.getUpdatedBy();
		val updatedByUsername = (updatedBy == null ? null : updatedBy.getUsername());
		val updatedDate = item.getUpdatedDate();
		return new AuditData(createdByUsername, createdDate, updatedByUsername, updatedDate);
	}
	...
	public List<MapItemDto> toDtoList(List<? extends Item> items, @NonNull ProjectionNode projectionNode) {
		...
		for (val item : items) { itemDtos.add(toDto(item, projectionNode)); }
		return itemDtos;
	}
