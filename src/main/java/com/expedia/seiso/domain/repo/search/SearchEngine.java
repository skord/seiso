...
public class SearchEngine<T extends Item, ID extends Serializable> {
	...
	private class PagedSearchTask implements Callable<PagedSearchResult<T>> {
		...
		private PagedSearchTask(@NotNull ItemAssembler itemAssembler,
				@NotNull SearchableRepository<T, ID> searchableRepository, Set<String> searchTokens, Pageable pageable) {
	...
	@Autowired(required = false)
	public SearchEngine(@NotNull ItemAssembler itemAssembler, @NotNull Repositories repositories,
	...
	private Collection<PagedSearchTask> createPagedSearchTasks(Set<String> searchTokens, Pageable pageable,
			Map<String, PagedResources<MapItemDto>> searchResultsByEntityType) {
		...
		while (entityTypeIterator.hasNext()) {
			...
			if (repository instanceof SearchableRepository) {
				pagedSearchTasks.add(new PagedSearchTask(this.itemAssembler, (SearchableRepository<T, ID>) repository,
						searchTokens, pageable));
			}
		}
	...
	public Map<String, PagedResources<MapItemDto>> search(TokenizedSearchQuery tokenizedSearch, Pageable pageable)
		...
		if (tokenizedSearch != null) {
			...
			if (!CollectionUtils.isEmpty(searchTokens)) {
				Collection<PagedSearchTask> pagedSearchTasks = this.createPagedSearchTasks(searchTokens, pageable,
						searchResultsByEntityType);
				List<Future<PagedSearchResult<T>>> pagedSearchTaskResults = this.executorService
						.invokeAll(pagedSearchTasks);
				...
				for (Future<PagedSearchResult<T>> pagedSearchTaskResult : pagedSearchTaskResults) {
					...
					searchResultsByEntityType.put(pagedSearchResult.resultType.getSimpleName().toLowerCase(),
							pagedSearchResult.result);
				}
			}
		}
